...
$('.glyphicon-fire').on('click', function(event) {
    event.preventDefault();
    postId = event.target.parentNode.parentNode.parentNode.parentNode.dataset['postid'];
    var isLike = event.target.previousElementSibling == null;
    $.ajax({
        ...
        .done(function() {
            event.target.innerText = isLike ? event.target.innerText == 'Like' ? 'Remove like' : 'Like' : event.target.innerText == 'Dislike' ? 'Remove dislike'  : 'Dislike';
